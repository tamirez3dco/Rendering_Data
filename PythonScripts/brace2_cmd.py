...
def run(ring_rad_diff, n_ring_divisions, n_circle_divisions, in_circle_rad, out_circle_rad, div_pipe_rad, in_pipe_rad, out_pipe_rad, division_shift):
    ...
    out_circle_rad = (7*out_circle_rad)+1
    ...
    for i in range(1,n_ring_divisions):
        new_pipes = rs.RotateObjects(pipes, (0,0,0), i*(360/n_ring_divisions), axis=axis, copy=True)
...
def RunCommand( is_interactive ):
    ...
    a4_o = Rhino.Input.Custom.OptionDouble(0.6)
    a5_o = Rhino.Input.Custom.OptionDouble(0.5)
    a6_o = Rhino.Input.Custom.OptionDouble(0.5)
    a7_o = Rhino.Input.Custom.OptionDouble(0.2)
